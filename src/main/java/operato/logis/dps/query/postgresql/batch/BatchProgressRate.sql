WITH T_JOBS AS (
	...
	FROM (
		SELECT 
			...
			O.COM_CD,
			O.SKU_CD,
			O.ORDER_QTY AS PICK_QTY,
			COALESCE(J.PICKED_QTY, 0) AS PICKED_QTY
		FROM 
			ORDERS O LEFT 
			OUTER JOIN 
			(SELECT * FROM JOB_INSTANCES WHERE DOMAIN_ID = :domainId AND BATCH_ID = :batchId AND STATUS NOT IN ('C','D')) J
			ON O.BATCH_ID = J.BATCH_ID AND O.CLASS_CD = J.CLASS_CD AND O.SKU_CD = J.SKU_CD
		WHERE 
